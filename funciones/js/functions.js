...
array_lengths.push( findLongestWord( ["Dog", "Cat", "Snake", "Crocodile", "Lion", "Tiger", "Bat", "Camel", "Pig"] ) );
	...
	for (i = 0; i < array.length; i++) {
		if (typeof array[i] === 'number' && array[i] > 9 ) {
			while (array[i] > 9) {
				...
				var stringArray = numToString.split('');
				var resString = 0;
				for (j = 0; j < stringArray.length; j++) {
					resString += parseInt(stringArray[j]);
